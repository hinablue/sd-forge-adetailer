...
from lib_adetailer.common import create_bbox_from_mask, create_mask_from_bbox, ThreadWithReturnValue
from lib_adetailer.logger import logger_adetailer as logger

from threading import Thread

def thread_safe_predict(func, image, confidence):
...
def mediapipe_predict(
    ...
    if model_type in mapping:
        ...
        predict = ThreadWithReturnValue(target=thread_safe_predict, args=(func, image, confidence,))
        predict.start()

        return predict.join()

    logger.error(f"Invalid mediapipe model type: {model_type}, Available: {list(mapping.keys())!r}")
