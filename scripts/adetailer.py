...
from modules import images, script_callbacks, safe, shared, paths, scripts, sd_models, shared_items, devices
...
from lib_adetailer.ui import ADetailerUiGroup
from lib_adetailer.logger import logger_adetailer as logger
from lib_adetailer.args import ADetailerUnit, WebuiInfo, SCRIPT_DEFAULT
...
from lib_adetailer.args import BBOX_SORTBY
...
class ADetailerScript(scripts.Script):
    ...
    def postprocess_image(self, p: StableDiffusionProcessing, pp: PostprocessImageArgs, *args):
        ...
        with pause_total_tqdm():
            for i, unit in enumerate(enabled_units):
                processed |= self.afterdetailer_process_image(i, unit, p, pp, args)
    ...
    @torch.no_grad()
    def afterdetailer_process_image(self, n: int, unit: ADetailerUnit, p, pp, *args):
        ...
        with change_torch_load():
            pred = predictor(ad_model, pp.image, unit.ad_detection_confidence_threshold, **kwargs)

        masks = self.pred_preprocessing(pred, unit)
        ...
        noise_multiplier = self.get_initial_noise_multiplier(p, unit)
        seed, subseed = self.get_seed(p)
        width, height = self.get_width_height(p, unit)
        sampler_name = self.get_sampler(p, unit)
        steps = self.get_steps(p, unit)
        cfg_scale = self.get_cfg_scale(p, unit)
        override_settings = self.get_override_settings(p, unit)

        i2i = StableDiffusionProcessingImg2Img(
            init_images=[pp.image],
            resize_mode=0,
            denoising_strength=unit.ad_detection_confidence_threshold,
            mask=None,
            mask_blur=unit.ad_inpaint_mask_blur,
            inpainting_fill=1,
            inpaint_full_res=unit.ad_inpaint_only_masked,
            inpaint_full_res_padding=unit.ad_inpaint_only_masked_padding,
        ...
        i2i.scripts, i2i.script_args = self.script_filter(p, unit)
        ...
        ad_prompts, ad_negatives = self.get_prompt(p, unit)
    ...
    def pred_preprocessing(self, pred: PredictOutput, unit: ADetailerUnit):
    ...
    def script_filter(self, p, unit: ADetailerUnit):
        ...
        if unit.ad_controlnet_model != "None":
    ...
    def get_prompt(self, p, unit: ADetailerUnit) -> tuple[list[str], list[str]]:
        i = self.get_i(p)
        prompt_sr = p._ad_xyz_prompt_sr if hasattr(p, "_ad_xyz_prompt_sr") else []

        prompt = self._get_prompt(unit.ad_prompt, p.all_prompts, i, p.prompt, prompt_sr)
        negative_prompt = self._get_prompt(
            unit.ad_negative_prompt,
    ...
    def get_model_mapping():
        ...
        if (
            ...
            os.makdir(adetailer_dir, mode=0o755)
...
script_callbacks.on_after_component(ADetailerUiGroup.on_after_component)
